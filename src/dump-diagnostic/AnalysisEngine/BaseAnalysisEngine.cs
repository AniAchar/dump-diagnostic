...
namespace AnalysisEngine
{
    public class BaseAnalysisEngine : IDisposable
    {
        ...
        protected List<AnalysisThread> GetThreadDetails()
        {
            List<AnalysisThread> analysisThreads = new List<AnalysisThread>();
            foreach (var thread in Runtime.Threads)
            {
                var at = new AnalysisThread
                {
                ...
                at.SetApartment(thread);
                at.SetException(thread);
                at.SetFinalizer(thread);
                at.SetGCMode(thread);
                analysisThreads.Add(at);
            }
    ...
    public class AnalysisThread
    {
        ...
        public void SetApartment(ClrThread t)
        {
            if (t.IsSTA)
            {
                Apartment = "STA";
        ...
        public void SetException(ClrThread t)
        {
            ClrException currentEx = t.CurrentException;
            if (currentEx is ClrException ex)
                CurrentException = $"Exception: {ex.Type.Name}, HResult= {ex.HResult}";
        }

        public void SetFinalizer(ClrThread t)
        {
            if (t.IsFinalizer)
                Finalizer = "Finalizer thread";
        }

        public void SetGCMode(ClrThread t)
        {
            GCMode = t.GCMode.ToString("G");
        }
